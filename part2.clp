...
(defrule to-route-btwn-packages 
  (package ? ? ?end-city $?)
  (package ? ?start-city $?)
  =>
  (assert (to-route ?end-city ?start-city)))

; (defrule to-route-btwn-packages-rev 
;   (package ? ?start-city $?)
;   (package ? ? ?end-city $?)
;   =>
;   (assert (to-route ?start-city ?end-city)))
...
(defrule start-truck-delivery
  ...
  ;; Get the available truck
  ?available-truck <- (truck ?number ?current-location none ?space-avail ?status none ?)
  (package ?package-number ?package-location ?package-dest ?package-size $? ?order-arrival-time ?)
  
  ;; make sure this truck isn't doing anything else
  (test (eq ?status idle))
  ...
  ;; Now we want to make sure no other truck can get there faster
  (forall
   (truck ?other-number ?other-location none ?space-avail2&:(>= ?space-avail2 ?package-size) idle $?) 
   (fastest-path ?other-location $? ?package-location ?other-eta)
   ;; Note the <= this makes it work, because it suceeds the test even if it matches itself
   (test (<= ?eta ?other-eta)))
  ...
  (if (eq idle ?status)
      then
      (bind ?new-status dispatched)
      else
      (bind ?new-status ?status))
  ...
  (assert (truck ?number ?current-location ?package-location
                 ...
                 ?new-status ?package-number ?new-eta)))
...
(defrule deliver-package
  ...
  (assert
   (truck ?number ?dest ?dest (+ ?space-avail ?package-size) returning none ?new-eta))
...
;(do-for-all-facts ((?p package)) TRUE (ppfact ?p))





;(do-for-all-facts ((?p truck)) TRUE (ppfact ?p))
